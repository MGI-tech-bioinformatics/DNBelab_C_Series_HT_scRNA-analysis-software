# 🧬 DNBelab C Series HT scRNA 分析结果文档

<div align="center">

**单细胞RNA测序分析输出文件完整指南**

[📁 目录结构](#-输出目录结构) • [📋 文件详情](#-文件详细说明) • [📄 格式说明](#-文件格式说明) • [📊 报告解读](#-网页报告释义)
...
单细胞RNA分析完成后，会在指定的输出目录中生成标准化的文件和子目录结构。本文档详细说明了每个输出文件的内容、格式和用途，帮助用户充分理解和高效利用分析结果。

> 💡 **提示**: 所有输出文件均采用标准格式，兼容主流单细胞分析工具（如Scanpy、Seurat等）。
...
| 📊 **分析结果** | 聚类、标记基因、质控数据 | [analysis/目录](#-分析结果目录-analysis) |
| 🧬 **比对文件** | BAM文件及索引 | [比对数据](#-比对和注释文件) |
| 📈 **表达矩阵** | 原始和过滤后的基因表达数据 | [矩阵文件](#-特征矩阵文件) |
| 📝 **指标汇总** | 质量控制和统计指标 | [汇总数据](#-分析指标汇总) |
| 📄 **格式规范** | 标准文件格式详解 | [格式说明](#-文件格式说明) |
...
> **核心内容**: 单细胞基因表达计数矩阵，分为原始数据和质控过滤后数据
...
### 📊 Market Matrix格式 (`.mtx.gz`) <a id="market-matrix格式-mtxgz"></a>
...
| **空间效率** | 稀疏格式节省存储，适合大规模数据 |
| **兼容性** | 支持Scanpy、Seurat、Bioconductor等工具 |
| **传输性** | 标准格式，便于数据交换和协作 |
| **性能** | 高效读写，支持并行处理 |
...
```python
import scanpy as sc
adata = sc.read_10x_mtx('filter_matrix/', cache=True)
...
```r
library(Seurat)
data <- Read10X(data.dir = "filter_matrix/")
seurat_obj <- CreateSeuratObject(counts = data)
```

---

### 🗃️ AnnData格式 (`.h5ad`) <a id="anndata格式-h5ad"></a>
...
# 查看数据结构
print(adata)
print(adata.obs.head())  # 细胞元数据
print(adata.var.head())  # 基因元数据
```
...
| 🏷️ **标签** | 📝 **描述** | 🎯 **用途** |
|-------------|-------------|-------------|
| `CB` | 细胞条形码标识符 (经错误校正) | 细胞归属 |
| `CC` | 经错误校正的细胞条形码序列 | 质量控制 |
| `CR` | 测序仪报告的原始细胞条形码 | 错误识别 |
| `CY` | 细胞条形码读取质量 (Phred分数) | 质量评估 |
| `UB` | 经错误校正的UMI序列 | 去重复 |
| `UR` | 测序仪报告的原始UMI序列 | 错误识别 |
| `UY` | UMI读取质量 (Phred分数) | 质量评估 |
...
| `GX` | 重叠基因ID | 基因归属 |
| `GN` | 重叠基因名称 | 基因注释 |
| `RE` | 区域类型 (E/N/I) | 外显子/内含子/基因间 |

#### 🚀 应用场景

| 🎯 **应用** | 📝 **说明** |
|-------------|-------------|
| **基因组浏览** | IGV/UCSC可视化对齐结果 |
| **重新分析** | 修改参数后重新处理 |
| **质量评估** | 对齐统计和质量控制 |
| **格式转换** | `bam2fastq`工具转回FASTQ |
