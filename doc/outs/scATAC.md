...
<p style="font-size: 21px; color: #86868b; margin: 0; font-weight: 400;">单细胞 ATAC 测序分析输出文件说明</p>
...
单细胞 ATAC 测序分析完成后，会在指定的输出目录中生成标准化的文件和子目录结构，用于染色质可及性分析和表观基因组学研究。

</div>

> **提示**：所有输出文件均采用标准格式，兼容主流单细胞表观基因组分析工具（如 Signac、ArchR 等），并遵循国际通用数据格式规范。
...
*   **内容与格式**:
    ...
    *   文件具体包含的 **5列信息** 如下表所示：
        ...
        <td>细胞 ID 标识符，对应 BAM 文件中的<code>CB</code>标签，用于将片段归属到特定细胞</td>
        </tr>
        <tr>
...
*   **核心用途**:
    ...
    *   **工具性能优化**: 被 ArchR、Signac、IGV 等工具用于加载和处理特定区域的数据。
*   **格式**:
...
*   **核心用途**:
    ...
    *   **信噪比提升**: 由于去除了低质量细胞和背景噪音，使用此文件可提高分析结果的准确性和信噪比。
*   **内容与格式**:
    *   文件格式与 `fragments.tsv.gz` 完全相同（压缩的 BED-like TSV），包含相同的 5 列信息。
    *   仅包含通过细胞过滤算法（例如，基于 TSS 富集和峰区域覆盖的片段数）被识别为"真实细胞"的片段。
...
*   **核心用途**:
    *   **下游分析支持**: 确保在使用过滤后的片段文件时，下游工具（如 ArchR, Signac）能够快速访问特定基因组区域的数据。
*   **格式**:
    *   由 `tabix` 工具生成的标准二进制索引文件。
...
*   **核心用途**:
    *   **比对结果查看与可视化**: 可用于 IGV 等基因组浏览器进行结果可视化，检查特定基因座的比对情况。
...
*   **核心用途**:
    *   **快速数据访问**: 允许 IGV、Samtools 等工具在无需完整加载 BAM 文件的情况下，快速跳转和读取任意基因组区域的比对数据。
...
*   **内容与格式**:
    *   包含三大类别的关键指标：
        ...
        <td>总读段对数、有效条形码比例、Q30 碱基质量等基础测序指标</td>
        </tr>
        <tr>
        <td align="left"><strong>细胞识别</strong></td>
        <td>估计细胞数量、峰区域片段占比、TSS 区域片段占比、峰检测数量、TSS 富集等细胞调用结果</td>
        </tr>
        <tr>
    ...
    *   内置推荐的质量控制标准，便于用户判断：
        ...
        <li><strong>TSS 富集分数（人/鼠）</strong>: >4</li>
        <li><strong>峰区域片段比例</strong>: >15%</li>
        <li><strong>TSS 区域片段比例</strong>: >10%</li>
...
*   **核心用途**:
    *   **结果可视化**: 以交互式图表展示质控结果、细胞聚类、TSS 富集等关键分析结果。
...
*   **文件组成**:
    *   一个完整的MEX格式数据由以下 **三个文件** 构成：
        ...
        <tr>
        <td align="left"><code>barcodes.tsv.gz</code></td>
        <td>压缩的细胞条形码文件。每行是一个细胞 ID，行号对应矩阵的<strong>列</strong>。</td>
        </tr>
        <tr>
        <td align="left"><code>peaks.bed.gz</code></td>
...
**概述**：HTML 网页报告提供单细胞 ATAC 测序结果的可视化摘要和指标说明，覆盖实验质量与下游分析相关的关键指标。
...
HTML 网页报告用于查看单细胞 ATAC 测序分析结果，覆盖细胞识别、片段质量、TSS 富集、peak 相关指标和聚类结果等内容。用户可通过交互式图表快速检查实验质量并定位需要进一步复核的指标。
...
<li><strong>计算过程</strong>: 合并同液滴的细胞条形码后，基于峰区域片段数量、TSS 比例等指标进行细胞过滤。</li>
<li><strong>判读建议</strong>: 该值应结合上样细胞数量、片段数分布和细胞过滤结果综合判断。若明显低于预期，建议检查样本完整性、文库质量和测序深度。</li>
</li>
</ul>
</td>
</tr>
...
<td>
<ul>
<li><strong>定义</strong>: 单个细胞内所包含的有效 ATAC-seq 片段数量的中位数。</li>
<li><strong>用途</strong>: 用于评估单个细胞中可用于开放染色质分析的有效片段数量和测序覆盖水平。</li>
...
<li><strong>判读建议</strong>: 建议此值 ≥ 25,000，以保证足够的染色质覆盖。该值偏低时，peak calling、聚类和差异可及性分析稳定性可能下降。</li>
</ul>
</td>
</tr>
<tr>
...
<li><strong>用途</strong>: 这是评估 ATAC 信号特异性的关键指标。比例越高，说明有效片段越集中于开放染色质区域，背景噪音相对较低。</li>
<li><strong>判读建议</strong>:
<ul><li><strong>需关注</strong>: < 15% 时，建议检查细胞核质量、转座反应效率、peak calling 结果和测序深度。</li></ul>
</li>
</ul>
</td>
</tr>
...
<li><strong>用途</strong>: 用于评估启动子附近开放染色质信号和 ATAC-seq 文库特异性。</li>
<li><strong>判读建议</strong>:
<ul><li><strong>需关注</strong>: < 10% 时，建议结合 TSS 富集曲线、片段长度分布和样本处理流程进行判断。</li></ul>
</li>
</ul>
</td>
</tr>
...
<td>
<ul>
<li><strong>定义</strong>: 在所有有效片段中，成功归属于高质量细胞 ID 的片段所占的比例。</li>
<li><strong>用途</strong>: 用于评估有效片段中有多少比例来自高质量细胞，是判断背景片段和空液滴影响的重要指标。</li>
<li><strong>判读建议</strong>:
<ul><li><strong>需关注</strong>: 比例偏低可能提示背景片段较高、细胞过滤严格度异常、样本质量差或文库构建异常。</li></ul>
</li>
</ul>
</td>
</tr>
...
<li><strong>用途</strong>: 用于评估样本中可检测开放染色质区域的数量和调控元件覆盖范围。</li>
<li><strong>影响因素</strong>: 受细胞数量、细胞类型异质性、测序深度和 peak calling 参数影响。</li>
...
<li><strong>用途</strong>: 用于评估细胞条形码识别是否稳定，直接影响片段能否正确归属到细胞。</li>
<li><strong>判读建议</strong>: 比例过低通常提示条形码识别异常，可能与条形码区域测序质量、污染或文库构建质量有关。</li>
</ul>
</td>
</tr>
<tr>
...
<li><strong>判读建议</strong>:
<ul><li><strong>需关注</strong>: < 50% 时，建议检查样本污染、物种选择、参考基因组版本和测序质量。</li></ul>
</li>
</ul>
</td>
</tr>
...
<li><strong>用途</strong>: 用于评估线粒体来源片段占比，辅助判断细胞核制备和样本完整性。</li>
<li><strong>判读建议</strong>: 比例过高（如 > 10%）通常提示细胞死亡、裂解过度或线粒体 DNA 捕获偏高。</li>
</ul>
</td>
</tr>
<tr>
...
<li><strong>用途</strong>: 用于评估开放染色质来源片段占比，反映 ATAC-seq 有效信号强度。</li>
<li><strong>判读建议</strong>: 高比例（如 > 40%）通常表示染色质可及性信号较好、转座反应效率较高。</li>
</ul>
</td>
</tr>
<tr>
...
<li><strong>用途</strong>: 用于评估单核小体片段占比，并与无核小体区域比例共同判断染色质结构完整性。</li>
</ul>
</td>
</tr>
<tr>
...
<li><strong>用途</strong>: Q30 表示碱基测序错误率低于 0.1%。该指标用于评估细胞条形码识别的基础准确性。</li>
</ul>
</td>
</tr>
<tr>
...
<li><strong>用途</strong>: 用于评估测序读段本身的质量水平，是后续比对、片段识别和 peak calling 准确性的基础。</li>
</ul>
</td>
</tr>
</tbody>
...
*   **坐标轴**:
    *   **X 轴 (Barcode Rank)**: 所有细胞条形码按片段数降序排列。左侧为高片段数细胞，右侧为低片段数细胞。
    *   **Y 轴 (Fragment Counts)**: 每个细胞对应的峰区域片段总数（对数刻度）。
*   **关键特征 (Knee Point)**:
    *   图中曲线通常会出现明显“拐点”（Knee Point）。
    *   拐点左侧的 **蓝色区域** 代表被算法识别为高质量真实细胞的条形码。
    *   拐点右侧的 **灰色区域** 代表背景噪音或低质量条形码。
*   **交互功能**:
    *   鼠标悬停可查看细胞的详细排序和片段数。
...
**图表功能**：
通过三个独立的小提琴图（Violin Plots），分别展示高质量细胞在 **片段数 (Fragments)**、**TSS 富集比例 (TSS Proportion)** 和 **Peak 区域片段比例 (Peak Proportion)** 这三个关键质量指标上的分布情况。
...
*   **各图解读**:
    *   **片段数 (Fragments)**: 展示细胞包含的片段总数分布。质量较好的文库通常具有较高的分布中心（最宽处）。
    *   **TSS 富集比例 (TSS Proportion)**: 展示 TSS 区域片段占比的分布。分布的中心越高，说明细胞整体的转录活性信号越好。
    *   **Peak 区域片段比例 (Peak Proportion)**: 展示峰区域片段占比的分布。分布的中心越高，说明信噪比越高。
...
**图表功能**：
展示去重后 ATAC-seq 片段的插入长度分布，用于评估样本质量和染色质结构完整性。
...
- **使用场景**: 用于判断文库复杂度和测序饱和度，辅助评估是否需要继续加深测序。  
- **判读建议**:
  - **高重复率**（如 >20-30%）通常表示测序深度已接近饱和，继续加深测序的收益可能有限。
  - **极低重复率**（如 <10%）可能表示测序深度不足，可考虑增加测序以获得更多唯一片段。

##### Jaccard threshold（Jaccard 相似度阈值）

- **定义**: 用于判断任意两个磁珠（Beads）是否来自同一细胞液滴的相似度阈值。  
- **使用场景**: 在 C4 ATAC 技术中，一个液滴可能包含多个磁珠，软件会根据磁珠间捕获片段的 Jaccard Index 判断是否合并。  
...
**核心功能**: 细胞聚类分析、TSS 富集模式、饱和度评估和磁珠相似性的高级可视化展示
...
1. **坐标轴**
    ...
    - **Y 轴**: 归一化后的信号强度（切割频率）。
2. **关键特征**
    - 高质量 ATAC-seq 数据通常在 TSS 中心（0 点）出现明显富集峰。
    - TSS 两侧信号会快速下降。
3. **质量评估**
    - **TSS Enrichment Score** 为该图的量化指标，分数越高（如 >4-6）通常表示信噪比越好、数据质量越高。
    - 若曲线较平坦、无明显富集峰，通常提示样本质量较差或实验流程存在问题。
