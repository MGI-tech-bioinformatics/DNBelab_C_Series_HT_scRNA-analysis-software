...
<p style="font-size: 21px; color: rgba(0,0,0,0.6); margin: 0 0 30px 0; font-weight: 400;">DNBelab C Series HT scRNA 参数配置完整指南</p>
...
<p><strong>选择一种输入方式：基于目录 OR 单独指定文件</strong></p>
...
<ul>
  <li>历史短参数 <code>-c1/-c2/-i1/-i2</code> 仍可使用，但在新版帮助信息中默认隐藏，建议优先使用长参数以便脚本可读性更好。</li>
</ul>
...
<ul>
  ...
  <li><strong>智能检测 (auto)：</strong> 推荐默认使用；仅在自动识别失败时手动指定。</li>
</ul>
<p><strong>默认值：</strong> <code>auto</code></p>
<p><strong>示例：</strong></p>
...
<ul>
  ...
  <li><strong>参数类型 (type)：</strong> <code>cb</code>（细胞条形码）、<code>umi</code>（UMI）、<code>R1</code>（Read1 有效序列）、<code>R2</code>（Read2 有效序列，仅双端测序）。</li>
...
<ul>
  ...
  <li><strong>影响：</strong> 较大的内存限制可以生成更高性能的索引，从而加快RNA分析速度，但会增加内存消耗；较小的内存限制会降低索引性能，可能导致分析速度变慢。</li>
...
<tbody>
<tr>
<td align="left" style="padding: 10px 16px;"><a href="../pipeline/scRNA.html">scRNA 流程文档</a></td>
<td style="padding: 10px 16px;">单细胞 RNA 分析流程指南</td>
</tr>
<tr>
<td align="left" style="padding: 10px 16px;"><a href="../outs/scRNA.html">scRNA 输出文档</a></td>
