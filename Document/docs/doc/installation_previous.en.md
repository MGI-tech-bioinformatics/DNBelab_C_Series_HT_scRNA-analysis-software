<div align="right" markdown="block">

[Home](../index.md)

</div>

<br>

# DNBelab C Series™ Previous Installation Guide

<br>

<div align="center" markdown="block">

**Installation guides and resources for historical versions of the DNBelab C Series™ HT Single-Cell Analysis Software**

</div>

<br>

---

<br>

## v3.x Series <a id="v3x-series"></a>

<table style="width:100%; border-collapse: collapse; margin: 1.5em 0; box-shadow: 0 2px 3px rgba(0,0,0,0.1);">
  <thead style="background-color: #f2f2f2; border-bottom: 2px solid #ddd;">
    <tr>
      <th style="padding: 12px 15px; border: 1px solid #ddd; text-align: left;">Version</th>
      <th style="padding: 12px 15px; border: 1px solid #ddd; text-align: left;">Release Date</th>
      <th style="padding: 12px 15px; border: 1px solid #ddd; text-align: left;">Installation Guide</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td style="padding: 12px 15px; border: 1px solid #ddd;"><b>3.0</b></td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;">Dec 18, 2025</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;"><a href="https://github.com/MGI-tech-bioinformatics/DNBelab_C_Series_HT_scRNA-analysis-software/blob/de7b77ee029ca668d6c197f22e6bb09431893f12/doc/installation.md">View Installation Guide</a></td>
    </tr>
  </tbody>
</table>

---

## v2.x Series <a id="v2x-series"></a>

The following v2.x versions are available for projects requiring reproducibility with previous stable pipelines.

<table style="width:100%; border-collapse: collapse; margin: 1.5em 0; box-shadow: 0 2px 3px rgba(0,0,0,0.1);">
  <thead style="background-color: #f2f2f2; border-bottom: 2px solid #ddd;">
    <tr>
      <th style="padding: 12px 15px; border: 1px solid #ddd; text-align: left;">Version</th>
      <th style="padding: 12px 15px; border: 1px solid #ddd; text-align: left;">Release Date</th>
      <th style="padding: 12px 15px; border: 1px solid #ddd; text-align: left;">Installation Guide</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td style="padding: 12px 15px; border: 1px solid #ddd;"><b>2.1.3</b></td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;">Oct 9, 2024</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;"><a href="https://github.com/MGI-tech-bioinformatics/DNBelab_C_Series_HT_scRNA-analysis-software/blob/version2.0/doc/installation.md">View Installation Guide</a></td>
    </tr>
    <tr>
      <td style="padding: 12px 15px; border: 1px solid #ddd;"><b>2.1.2</b></td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;">Apr 24, 2024</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;"><a href="https://github.com/MGI-tech-bioinformatics/DNBelab_C_Series_HT_scRNA-analysis-software/blob/d0f63d878be34f4b8bf0d169ebbecdbc11f0133e/doc/installation.md">View Installation Guide</a></td>
    </tr>
  </tbody>
</table>

---

## v1.x Series <a id="v1x-series"></a>

Legacy versions from the v1.x series are archived. Please refer to the GitHub repository for access.

<table style="width:100%; border-collapse: collapse; margin: 1.5em 0; box-shadow: 0 2px 3px rgba(0,0,0,0.1);">
  <thead style="background-color: #f2f2f2; border-bottom: 2px solid #ddd;">
    <tr>
      <th style="padding: 12px 15px; border: 1px solid #ddd; text-align: left;">Version</th>
      <th style="padding: 12px 15px; border: 1px solid #ddd; text-align: left;">Description</th>
      <th style="padding: 12px 15px; border: 1px solid #ddd; text-align: left;">Resource</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td style="padding: 12px 15px; border: 1px solid #ddd;"><b>1.x</b></td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;">Early releases (Legacy)</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;"><a href="https://github.com/MGI-tech-bioinformatics/DNBelab_C_Series_HT_scRNA-analysis-software/blob/version1.0/doc/installation.md">View GitHub Version Notes</a></td>
    </tr>
  </tbody>
</table>

---

<div style="background-color: #fffbe6; border-left: 6px solid #ffc107; padding: 15px; margin: 1.5em 0; border-radius: 4px;" markdown="block">
<strong>Note</strong>: We recommend using the latest stable release for all new projects. Previous versions are provided to support existing workflows and research reproducibility.
</div>
