...
p1 <- ggplot(sp, aes(x=Human_UB,y=Mouse_UB,color=Species_UMI)) + geom_point()  +scale_color_manual(values=c("Human" = "blue", "Mouse" =  "red", "Mix"="#999999")) + labs(title=paste("Multiplet rate = ", umi_mix_ratio, "%", sep="")) + xlab(paste("Human beads : ",sum['Human'],sep="")) + ylab(paste("Mouse beads : ",sum['Mouse'])) + annotate("text",x=x,y=y, label=paste("Mix beads : ", sum['Mix']), size=10,color="#999999") + theme(plot.title = element_text(color="red", size=24, face="bold.italic"), panel.background = element_blank(),axis.line = element_line(colour = "black"), axis.title.x = element_text(color="blue", size=24, face="bold.italic"), axis.title.y = element_text(color="red", size=24, face="bold.italic"))
