...
if (file.exists(paste(opt$output,"/cell_report_1.csv",sep=""))) {
    file.remove(paste(opt$output,"/cell_report_1.csv",sep=""))
}
cat(paste("Estimated Number of Cell,", CellNum, "\n",sep=""),file=paste(opt$output,"/cell_report_1.csv",sep=""))
cat(paste("Reads in cell,", round(in_cell,3)*100, "%\n",sep=""),file=paste(opt$output,"/cell_report_1.csv",sep=""), append=T)
